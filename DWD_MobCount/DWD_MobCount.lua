...
DWD_MobCountDB = DWD_MobCountDB or {}
...
local counted     = {}   -- [guid] = time when we already credited a kill (dup guard)
...
local function isCreatureGUID_hex(g)
  ...
  local up = g:upper()
  -- F130 (creature), F150 (vehicle); some forks use F530
...
-- Canonical WotLK extraction: NPC entry = hex chars 7..10 AFTER "0x" (WoWWiki method)
local function npcIdFromHexGUID(g)
  local up = g:upper()
  if not up:find("^0X") then return nil end
  local s = up:sub(3) -- strip "0x"
  local hx = s:sub(7, 10) -- 7..10 is the entry id
...
local function isCreatureGUID(g)
  return isCreatureGUID_hex(g) or isCreatureGUID_dash(g)
end

-- Identify "mine" strictly by GUID OR NAME (flags can be flaky on some cores)
...
-- ==== Tooltip: only show on ATTACKABLE mobs; always "Killed: #", default 0 ====
local function TooltipUnit(tt)
  local _, unit = tt:GetUnit()
  if not unit or not UnitExists(unit) then return end
  if UnitIsPlayer(unit) then return end
  if not UnitCanAttack("player", unit) then return end -- HIDE on own-faction/friendly NPCs
...
local function CreateConfigFrame()
  ...
  clear:SetScript("OnClick", function() StaticPopup_Show("DWDMOBCOUNT_CLEAR") end)
...
StaticPopupDialogs["DWDMOBCOUNT_CLEAR"] = {
  ...
  OnAccept=function()
    if charKey then DWD_MobCountDB[charKey]={killsById={},namesById={},killsByName={}} end
    ensureCharTables()
    if cfgFrame and cfgFrame:IsShown() then cfgFrame:Hide(); cfgFrame:Show() end
...
local function ShowConfig()
  ...
  local arr = {}
  for id,c in pairs(killsById) do
    table.insert(arr, {name=(namesById[id] or ("NPC "..id)), cnt=c})
  end
  for nm,c in pairs(killsByName) do
    table.insert(arr, {name=nm, cnt=c})
  end
...
local TAG_TTL = 20 -- seconds window for UNIT_DIED/name-text fallbacks
...
frame:SetScript("OnEvent", function(self, event, ...)
  ...
  elseif event == "PLAYER_LOGIN" or event == "PLAYER_ENTERING_WORLD" then
    ...
    if event == "PLAYER_LOGIN" then
      DEFAULT_CHAT_FRAME:AddMessage("|cffffff00MobCount:|r Loaded. /mobcount (config|clear|debug).")
    end
  ...
  elseif event == "COMBAT_LOG_EVENT_UNFILTERED" then
    ...
    -- cache name + map name->id when possible
    ...
    if subevent == "PARTY_KILL" and dstGUID and isCreatureGUID(dstGUID) and isMineSource(srcGUID, srcName) then
      ...
      if not counted[dstGUID] or (now - counted[dstGUID]) > 2 then
        ...
        if id then
          ...
          dprint("kill(PARTY_KILL) id", id, nm)
        else
          if nm ~= "" then killsByName[nm] = (killsByName[nm] or 0) + 1 end
          dprint("kill(PARTY_KILL) name", nm)
        end
        counted[dstGUID] = now
      end
      return
      ...
      if t and (GetTime() - t) <= TAG_TTL then
        ...
        if not counted[dstGUID] or (now - counted[dstGUID]) > 2 then
          ...
          if id then
            namesById[id] = nm
            killsById[id] = (killsById[id] or 0) + 1
            dprint("kill(UNIT_DIED) id", id, nm)
          else
            if nm ~= "" then killsByName[nm] = (killsByName[nm] or 0) + 1 end
            dprint("kill(UNIT_DIED) name", nm)
          end
          counted[dstGUID] = now
        end
      end
  ...
  elseif event == "COMBAT_TEXT_UPDATE" then
    ...
    if action == "KILLING_BLOW" and victim then
      ...
      if id then
        namesById[id] = victim
        killsById[id] = (killsById[id] or 0) + 1
        dprint("kb id", id, victim)
      else
        killsByName[victim] = (killsByName[victim] or 0) + 1
        dprint("kb name", victim)
      end
    end
    ...
    if victim then
      ...
      if taggedAt and (now - taggedAt) <= TAG_TTL then
        ...
        if id then
          namesById[id] = victim
          killsById[id] = (killsById[id] or 0) + 1
          dprint("hostile_death id", id, victim)
        else
          killsByName[victim] = (killsByName[victim] or 0) + 1
          dprint("hostile_death name", victim)
        end
      end
    end
  end
...
-- (redeclare ShowConfig here to capture cfgFrame)
local function ShowConfig()
  if not cfgFrame then cfgFrame = CreateConfigFrame() end
  ensureCharTables()
...
SlashCmdList["DWDMOBCOUNT"] = function(msg)
  ...
  elseif m == "debug" then
    ...
    DEFAULT_CHAT_FRAME:AddMessage("|cffffff00MobCount:|r /mobcount [config|clear|debug]")
  end
end
